...
class Monitor(object):
    ...
    def connect(self):
        ...
        if self.name=='':
            ...
            self.dao = sim_messages.__dict__[self.name+"_DAO"]
...
def load(configuration):
    ...
    LDPATH=os.path.dirname(__file__) +  "/../../scenarios/" + configuration + "/" +  os.uname()[0] + "_" + os.uname()[4]  + "/lib/"
    ctypes.cdll.LoadLibrary(LDPATH+"libsimscenario_SBIRS_GEO1.so")
    global scenario
    global sim_messages
    global sim_library
    if sim_library==None:
        sim_library=__import__("sim_library", globals(), locals())
    if scenario==None:
        scenario=sim_library.Sim_Library(LDPATH+"libsimscenario_SBIRS_GEO1.so")
    if sim_messages==None:
        modulename="sim_messages"
        sim_messages=__import__(modulename, globals(), locals(), ['System_Stats_IO'],-1)
        sim_messages.scenario=scenario
        sim_messages.init(scenario)
    Format.DEFAULT_FORMAT=Format("Default")
    global all_monitors
    global all_monitors_dict
    all_monitors=[]
    all_monitors_dict={}
    for item in dir(sim_messages):
        if item.endswith('_DAO') and (item != 'Base_DAO'):
            ...
            all_monitors_dict[monitor.get_name()]=monitor
    def compare(str1, str2):
        return str1 < str2
    all_monitors.sort(compare)
