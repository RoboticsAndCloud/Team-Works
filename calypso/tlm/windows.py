...
class TelemetryWindow(object):
    ...
    def __init__(self,user, name, display):
        ...
        for model in tile_models:
            ...
            properties=DisplayProperties.get_properties(parent=model, user=user, display=display)
            tile  = Tile(name=model.get_name(), user=user, parent=self, display=display, type=model.get_type())
            self.place_tile( tile, properties)
        c.all_monitors   = calypso.tlm.all_monitors
        c.all_monitors_dict   = calypso.tlm.all_monitors_dict
    ...
    def remove_col(self, col):
        for rowindex in range(len(self.tiles)):
            ...
            for colindex in range(col, len(row)):
                ...
                if self.tiles[rowindex][colindex]!=None:
    ...
    def move_tile(self, from_x_pos, from_y_pos, to_x_pos, to_y_pos):
        ...
        if (to_y_pos<len(self.tiles)) and (to_x_pos < len(self.tiles[to_y_pos])):
            ...
            to_tile=None
        print "   FROM " + str(self.tiles[from_y_pos][from_x_pos])
        print "   TO   " + str(to_tile)
    ...
    def get_tabs(display, user):
        ...
        windows=[]
        for model in models:
            window=TelemetryWindow( user=user, name=model.get_name(), display=display)
            window.colcount=0
            for tilerow in window.tiles:
                if (len(tilerow)>window.colcount): 
                    window.colcount=len(tilerow)
            window.rowcount=len(window.tiles)
